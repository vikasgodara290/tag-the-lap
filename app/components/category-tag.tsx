...
export default function CategoryTag({ taskCategory, category }: CategoryTagProps) {
  ...
  return (
    <div className={`flex items-center justify-center ${colors.bg} ml-8 rounded-sm pr-3 max-sm:ml-2`}>
      <Dot className={`relative bottom-px ${colors.dot}`} />
      <span className={`${colors.text} text-sm font-bold`}>{categoryObj.category}</span>
    </div>
  );
}
